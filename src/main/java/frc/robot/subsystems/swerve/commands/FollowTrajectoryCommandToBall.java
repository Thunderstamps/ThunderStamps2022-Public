...
public class FollowTrajectoryCommandToBall extends CommandBase {
    
    private final SwerveSubsystem swerveSubsystem;
    private final Trajectory trajectory;
    private PhotonVision photonVision;
    private double startVisionTarget_sToEnd;
    ...
    private final double BALL_X_PIXEL = 110;

    private double elapsedTime_sec;
    private double totalTime_sec;
    private double maxError_in;

    public FollowTrajectoryCommandToBall(
            SwerveSubsystem swerveSubsystem,
            Trajectory trajectory,
            PhotonVision photonVision,
            double startVisionTarget_sToEnd) {
        this.swerveSubsystem = swerveSubsystem;
        this.trajectory = trajectory;
        this.addRequirements(swerveSubsystem);
        this.photonVision = photonVision;
        this.startVisionTarget_sToEnd = startVisionTarget_sToEnd;
    }
    ...
    public void execute() {
        ...
        var robotYOffset = 0.0; 
        var targetData = this.photonVision.GetBallTargetData();
        if((this.totalTime_sec - this.elapsedTime_sec) <= startVisionTarget_sToEnd && targetData.ValidTargetData())
        {
            var X_PixelOffset = BALL_X_PIXEL - targetData.X_pixels();
            var gain = X_PixelOffset > 0.0 ? 0.8 : 1.0;
            robotYOffset = X_PixelOffset * gain;
            translation_in_sec = nominalTranslation_in_sec;
        }
        else{
        ...
        this.swerveSubsystem.executeAutonomousControl(
            translation_in_sec,
            Vector2D.FromXY(0, robotYOffset),
            0.0);
